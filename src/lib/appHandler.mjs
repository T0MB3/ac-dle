...
import { compareGuess } from "./gameLogic.mjs";
import { createGameId, readGameId } from "./gameSession.mjs";
import {
  buildSearchIndex,
...
export async function handleNodeRequest(req, res) {
  try {
    ...
    if (req.method === "POST" && req.url === "/api/guess") {
      return handleGuess(await readJsonBody(req), res);
    }
...
export default async function handleVercelRequest(req, res) {
  try {
    ...
    if (req.method === "POST" && pathname === "/api/new-game") {
      return handleNewGame(res);
    }

    if (req.method === "POST" && pathname === "/api/guess") {
      return handleGuess(req.body ?? {}, res);
    }
...
function handleNewGame(res) {
  ...
  const gameId = createGameId(answerCharacter.id);
...
function resolveAnswerCharacter(gameId) {
  const answerId = readGameId(gameId);
  if (answerId) {
